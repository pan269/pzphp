 ...
 */
        $C = strtolower( Register::get('router')['c']); //获取路径controller
        $A = strtolower(Register::get('router')['a']); //获取路径action
        $E = trim(strtolower(Register::get('config')['view']['ext']),'.'); //获取文件ext
        $P = APP_DIR.DS."view".DS.$C.DS;
        $F = $A.$E;
        /**
         ...
         *
         */
        defined('LIQUID_INCLUDE_SUFFIX') or define('LIQUID_INCLUDE_SUFFIX', $E);
        ...
        define('LIQUID_VIEW_PATH', $P); // 文件目录
...
class Liquid
{
     ...
     */
    public static function liquid_autoload($className)
    {
        echo $className;die;
